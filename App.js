...
export default function App() {
  ...
  return (
    <View style={styles.container}>
      <ScrollView>
      <Text style={styles.titulo}>Nuestros Eventos</Text>
      
      {/* {eventos.map(evento => { 
          {
            return(
                <Text>{evento.startDate}</Text>,
                <Text>{evento.title}</Text>
                // <Text>{evento.notes}</Text>
            )
        ...
        :eventos.map(evento => { count++
            ...
            /*   <View>
                <Text>Fecha: {fecha.getDate()+'/'+((fecha.getMonth().toString().length == 1)? "0"+fecha.getMonth():fecha.getMonth())+'/'+fecha.getFullYear()}</Text>
                <Text>Titulo: {eventos[3]?.title}</Text>
                <Text>Descripcion: {eventos[3]?.notes}</Text>
              </View> */
            
                <Card key={count} style={{borderWidth:1, marginVertical:20, marginHorizontal: 30, borderRadius: 20}}>
                  <Card.Title title={fecha.getDate()+'/'+((fecha.getMonth().toString().length == 1)? "0"+fecha.getMonth():fecha.getMonth())+'/'+fecha.getFullYear()}/>
                  <Card.Content>
        ...
        {/* UN SOLO EVENTO
          ...
          <Text> Cargando Eventos</Text>
        :(  //ISO 8601 
          fecha = new Date(eventos[3]?.startDate),
          <View>
            <Text style={styles.fecha}>Fecha: {fecha.getDate()+'/'+((fecha.getMonth().toString().length == 1)? "0"+fecha.getMonth():fecha.getMonth())+'/'+fecha.getFullYear()}</Text>
            <Text style={styles.subtitulo}>Titulo: {eventos[3]?.title}</Text>
            <Text style={styles.descripcion}>Descripcion: {eventos[3]?.notes}</Text>
          </View>
          )      */
        }
    </ScrollView>  
